...
<div class="cart-inner">
    <div class="cart-thead clearfix">
       <!--  <div class="column t-checkbox form"><input data-cart="toggle-cb" name="toggle-checkboxes" id="toggle-checkboxes_up" type="checkbox" checked="" value=""><label for="toggle-checkboxes_up">全选</label></div> -->
        ...
        <div class="column t-price">物空价</div>
    ...
    <div id="product-list" class="cart-tbody">
        ...
        <c:set var="showItemNum" value="0"></c:set>
        <c:forEach items="${cartList}" var="cart">
        	<c:set var="totalPrice"  value="${ totalPrice + (cart.price * cart.num)}"/>
        	<c:set var="showItemNum" value="${showItemNum + cart.num }"/>
	        <div id="product_11345721" data-bind="rowid:1" class="item item_selected ">
		        <div class="item_form clearfix">
		            <div class="cell p-checkbox"><input data-bind="cbid:1" class="checkbox" type="checkbox" id="checkItem" name="checkItem" checked="" value="${cart.id }"/></div>
		            <div class="cell p-goods">
		                <div class="p-img">
		                	<a href="http://localhost:8074/item/${cart.id }.html" target="_blank">
		                		<img clstag="clickcart|keycount|xincart|p-imglistcart" src="${cart.image}" alt="${cart.title}" width="52" height="52">
		                	</a>
		                </div>    
		                <div class="p-name">
		                	<a href="http://localhost:8074/item/${cart.id }.html" clstag="clickcart|keycount|xincart|productnamelink" target="_blank">${cart.title}</a>
		            ...
		            <input type="hidden" value="${cart.price / 100}" id="itemPrice${cart.id }"/>
		            ...
		            <div class="cell p-quantity" for-stock="for-stock-11345721">
		                <div class="quantity-form" data-bind="">
		                    ...
		                    <input type="text" class="quantity-text" itemPrice="${cart.price}" itemId="${cart.id}" value="${cart.num }" id="num${cart.id }">
          ...
          <div class="cart-toolbar clearfix">
            <div class="total fr">
                <p><span class="totalSkuPrice" id="totalSkuPrice">¥<fmt:formatNumber value="${totalPrice / 100}" maxFractionDigits="2" minFractionDigits="2" groupingUsed="true"/></span>总计：</p>
                <p><span id="totalRePrice">- ¥0.00</span>优惠：</p>
            </div>
            <div class="amout fr"><span id="selectedCount">${showItemNum }</span> 件商品</div>
        </div>
        <div class="ui-ceilinglamp-1" style="width: 988px; height: 49px;"><div class="cart-dibu ui-ceilinglamp-current" style="width: 988px; height: 49px;">
          <div class="control fdibu fdibucurrent">
              ...
              <span class="delete">
                  ...
                  <a href="javascript:void(0);" clstag="clickcart|keycount|xincart|clearcartlink" id="remove-batch" onclick="deleteCheckItem()">
                          删除选中的商品
                  </a>
              </span>
              <span class="shopping">
          ...
          <div class="cart-total-2014">
              <div class="cart-button">
                  <span class="check-comm-btns" id="checkout-jd">
                      <a class="checkout" href="javascript:void(0);" clstag="clickcart|keycount|xincart|gotoOrderInfo" id="toSettlement" onclick="goOrderCart()">去结算<b></b></a>
                  </span>
              ...
              <div class="total fr">
                  总计（不含运费）：
                  <span class="totalSkuPrice" id="totalSkuPrice2">¥<fmt:formatNumber value="${totalPrice / 100}" maxFractionDigits="2" minFractionDigits="2" groupingUsed="true"/></span>
              </div>
          </div>
      </div></div>
</div><!-- cart-inner结束 -->
...
<script type="text/javascript">
	function deleteCheckItem(){
		var item = $("[name='checkItem']:checked");
		itemIds = [];
		itemNums = [];
		for(id in item){
	        if(item[id].checked){
	        	itemIds.push(item[id].value);
	        	itemNums.push($("#num"+item[id].value).val());
	        }
	    }
		var json = JSON.stringify(item_json);  
	...
	function goOrderCart(){
	    ...
	    window.location.href="http://localhost:8069/order/order-cart.html?items=" + itemIds + "," + itemNums;
	}
	$(function(){
	    select();	    
	})
	function select(){
		...
		for(id in item){
        if(item[id].checked){
        	tolprice =tolprice + $("#itemPrice" + item[id].value).val() * $("#num"+item[id].value).val();
        	}
    	}
		$("#totalSkuPrice").html(tolprice);
		$("#totalSkuPrice2").html(tolprice);
    });  
	}

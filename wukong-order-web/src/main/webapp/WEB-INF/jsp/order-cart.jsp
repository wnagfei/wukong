...
<div class="w w1 header clearfix">
    <div id="logo"><a href="http://localhost:8078"><img width="270" height="60" src="/images/wukong-logo.gif" alt="物空商城"></a></div>
</div>
...
<form id="orderForm" class="hide" action="/order/create.html" method="post">
		...
		<input id="payment" type="hidden" name="payment" value="<fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${totalPrice/100 }"/>"/>
		<input id="receiverName" type="hidden" name="orderShipping.receiverName" />
		<input id="receiverMobile" type="hidden" name="orderShipping.receiverMobile" />
		<input id="receiverState" type="hidden" name="orderShipping.receiverState" />
		<input id="receiverCity" type="hidden" name="orderShipping.receiverCity" />
		<input id="receiverDistrict" type="hidden" name="orderShipping.receiverDistrict" />
		<input id="receiverAddress" type="hidden" name="orderShipping.receiverAddress" />
		<input id="totalPrice" name="totalPrice" value="${ totalPrice }"/>
</form>
...
<div class="step-tit">
	<h3>收货人信息</h3>
	<div class="extra-r">
		<a href="javascript:void(0)" class="ftx-05" onclick="checkAddress()">新增收货地址</a>
	</div>
</div>
<div class="step-cont">
	<div class="consignee-list" id="consignee-list1">
		...
		<div id="consignee1" class="list-cont ui-switchable-body">
            <div id="consignee-ret"></div>
   			...
   			<c:forEach items="${receiver }" var="r" varStatus="status">
				<!---->
				<div id="showAddress${r.id }">
				<li class="ui-switchable-panel" id="consignee_index_137617472"
					...
					<div class="consignee-item item-selected"
						...
						<div class="user-name">
							<div class="fl">
								<strong limit="4">${r.receiverName }</strong>&nbsp;&nbsp;收
							</div>
							<div class="fr">${r.receiverMobile }</div>
							<div class="clr"></div>
						</div>
						<div class="mt10" limit="15">${r.receiverState }&nbsp;${r.receiverCity }&nbsp;${r.receiverDistrict }</div>
						<div class="adr-m" limit="30">${r.receiverAddress }</div>
						<div class="op-btns ar">
							<a href="javascript:void(0)}" onclick="selectAddress(${r.id})"	class="ftx-05 mr10 edit-consignee" fid="137617472">设为收货地址</a>
						
							<a href="javascript:void(0)" onclick="deleteAddress(${r.id})" class="ftx-05 mr10 edit-consignee"	fid="137617472">删除</a>
						</div>
				...
				<!---->
				<input id="sreceiverName${r.id}" type="hidden" value="${r.receiverName }"/>
				<input id="sreceiverMobile${r.id}" type="hidden" value="${r.receiverMobile }"/>
				<input id="sreceiverState${r.id}" type="hidden" value="${r.receiverState }"/>
				<input id="sreceiverCity${r.id}" type="hidden" value="${r.receiverCity }"/>
				<input id="sreceiverDistrict${r.id}" type="hidden" value="${r.receiverDistrict }"/>
				<input id="sreceiverAddress${r.id}" type="hidden" value="${r.receiverAddress }"/>
				...
				<li style="cursor: pointer;" onclick="save_Pay(1);">
					<div class="payment-item item-selected online-payment "
						...
						<b></b> 货到付款<span class="qmark-icon qmark-tip"
							data-tips="送货上门后再收款，支持微信，支付宝，现金、刷卡、支票 <a href='#' target='_blank' class='ftx-05'>查看服务及配送范围</a>"></span>
					</div>
				</li>
				...
				<li style="cursor: pointer;" onclick="save_Pay(5); ">
					...
					<div class="payment-item online-payment "
						...
						<b></b> 公司转账 <span class="qmark-icon qmark-tip"
							data-tips="通过快钱平台转账 转帐后1-3个工作日内到帐 <a href='#' target='_blank' class='ftx-05'>查看帐户信息</a>"></span>
					</div>
				</li>
				<li style="cursor: pointer;" onclick="save_Pay(2); ">
					...
					<div class="payment-item  online-payment "
						...
						<b></b> 邮局汇款 <span class="qmark-icon qmark-tip"
							data-tips="通过快钱平台收款 汇款后1-3个工作日到账 <a href='#' target='_blank' class='ftx-05'>查看帮助</a>"></span>
					</div>
				</li> 
				<!--div id="shipment"></div--> 
				<script>
...
<div class="shopping-list ABTest">
	<div class="goods-list">
     <!--配送方式-->
    <h4 class="vendor_name_h" id="0">商家：物空商城</h4>		         
		...
		<div class="mode-tab-nav">
			<ul>
				<li class="mode-tab-item " id="jd_shipment_item"
					...
					id="jdShip-span-tip" class="m-txt">物空快递<i
						class='qmark-icon qmark-tip'
						data-tips='由物空公司负责配送，速度很快，还接受上门刷卡付款服务'></i></span><b></b></li>
			</ul>
		</div>
	</div>
</div>
    ...
    <div class="ui-ceilinglamp checkout-buttons">
      ...
      <div class="sticky-wrap">
          ...
          <button type="button" class="checkout-submit btn-1" 
          		  id="order-submit"	onclick="isAddress()">
          	提交订单
          </button>
          ...
          <div style="display:none" id="factoryShipCodShowDivBottom" class="dispatching">
            部分商品货到付款方式：先由物空配送“提货单”并收款，然后厂商发货。
          </div>
        </div>
...
<!-- /main -->
	...
	<script type="text/javascript">
		...
		function checkAddress(){
			$.ajax({
			url : "/order/checkAddress.action",
			type :"post",
			success : function(data){
				if(data.status == 200){
					location.href = "/order/addAddress.html";
				}
				else{
					alert("收货地址过多，请先删除已有收货地址!!!");
				}
			}
		})
		}
		...
		function deleteAddress(id){
			$.ajax({
			url : "/order/deleteAddress.action",
			data : {
				id : id
			},
			type :"post",
		 ...
		 function selectAddress(id){
			 $("div[id*='showAddress']").hide();
			 $("#showAddress"+id).show();
			 $("#receiverName").val($("#sreceiverName"+id).val());
			 $("#receiverMobile").val($("#sreceiverMobile"+id).val());
			 $("#receiverState").val($("#sreceiverState"+id).val());
			 $("#receiverCity").val($("#sreceiverCity"+id).val());
			 $("#receiverDistrict").val($("#sreceiverDistrict"+id).val());
			 $("#receiverAddress").val($("#sreceiverAddress"+id).val());
